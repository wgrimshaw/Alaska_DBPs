...
library(dplyr)
library(lubridate)
newdata <-mutate(newdata, DOY = yday(DATE))
```
...
PrecipMedian <- newdata%>% group_by(Bin, DOY) %>%
  mutate(PrecipMedian= median(PRCP))
...
>Do Seasonal Mann Kendall test for the 9 stations 
>Moving average from coding challenge
