...
```{r, fig.cap= c("Map of NOAA Site Locations in Alaska"), fig.height=5, fig.width=8, warning=FALSE, message=FALSE}
ggplot() +
  geom_sf(data = Alaska_sf_utm, color = "black") +
  geom_sf(data = NOAA_sf_utm, color = "red")+
  geom_sf(data = DischargeComplete_utm, color="blue", size=2) +
gabytheme
```
add in all 18 site locations for temp and precip in this map 
no colors-just red
discharge 
use same shape type: orange dots are NOAA sites, blue dots are NWIS discharge 
export precipitation map and total site locations map 
...
AlaskaPrecipMap<-ggplot(axes = FALSE) +
  ...
  geom_sf(data = AlaskaMeanPrecipDischargeSummary_utm, aes(color=Mean_PRCP), size=7)  +
...
AlaskaDischargeMap<-ggplot(axes = TRUE) +
  ...
  geom_sf(data = DischargeComplete_utm, aes(color=Mean_Discharge), size=7)  +
...
AlaskaTemperatureMap<-ggplot(axes = TRUE) +
  geom_sf(data = Alaska_sf_utm, fill="white") +
  geom_sf(data = AlaskaMeanPrecipDischargeSummary_utm, aes(color=Mean_Max_Temp), size=7)  +
...
>>>>>>> c01dd2fd19e7bfd2b909afea315805aa23f144b3
```{r}
...
# Seasenol Analysis of Discharge 
```{r}
...
# Normalize the slope by divided by mean discharge over time of each bin
meanDischarge <- meanDischarge%>%
  mutate(meanSlope = Slope/meanDischarge)

# write csv file of Bin and mean sens.slope of discharge
...
```
>From Seasonal Mann Kendall test and the normalized sens.slope of discharge in each bin, except for bin 1 and 3, discharges of other bins show small seasonal change and they are slightly increasing over time.
>>>>>>> c01dd2fd19e7bfd2b909afea315805aa23f144b3
